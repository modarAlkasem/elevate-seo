...
export const SourceDistributionChart = ({
  ...
  /** processes the source type data from SEO report into format suitable for Pie chart
   * - Extracts source types and their counts from seoReport.inventory.source_types
   * - Assigns colors from pre-defined palette to each source type
   * - Returns an array of objects with name, value (count), and color for each source type
   * - Returns an empty array of objects if no valid source types found
   */
  const sourceTypeEntries = (() => {
    ...
    // if no source types found, return an empty array
    ...
    // return an array objects with name, value, and color for each source type
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription className="text-base">
              Breakdown of data source types and volume
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          <div className="space-y-4">
            ...
            {sourceTypeEntries.map((entry, idx) => (
              <div
                key={idx}
