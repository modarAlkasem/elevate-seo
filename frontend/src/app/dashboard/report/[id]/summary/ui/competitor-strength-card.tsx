...
type CompetitorStrengthCardProps = {
  seoReport?: SeoReport;
};
export const CompetitorStrengthCard = ({
  seoReport,
}: CompetitorStrengthCardProps) => {
  const competitorStrength = (seoReport?.competitors || [])
    .filter((c) => c.name && c.strength_score)
    .map((c) => ({
      name: c.name,
      ...
      <CardHeader className="pb-6">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-full bg-orange-100 dark:bg-orange-900/50">
            <Users className="h-5 w-5 text-orange-600 dark:bg-orange-400" />
          </div>
          <div>
